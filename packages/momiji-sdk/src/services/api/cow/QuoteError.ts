...
export class GpQuoteError extends MomijiError {
	...
	public constructor(quoteError: GpQuoteErrorObject) {
	...
	public static quoteErrorDetails = GpQuoteErrorDetails;
	...
	public static async getErrorFromStatusCode(response: Response) {
		switch (response.status) {
			...
			case 500:
			default:
				log.error(logPrefix, '[QuoteError::getErrorFromStatusCode] Error fetching quote, status code:', response.status || 'unknown');
