...
export function useSmartContractTransaction<TContract extends Contract, TMethodName extends ContractMethodName<TContract>>(
	...
	return useMutation({
		mutationFn: async (args: ContractMethodArgs<TContract, TMethodName>): Promise<ContractReceipt> => {
			...
			if (!contract) {
				...
				console.warn(`Tried to call ${methodName as string} without contract instance.`);
				return undefined as unknown as ContractReceipt;
			}
			...
			if (setGasLimit) {
				const gasEstimate = await fetchGasEstimate<TContract, TMethodName>(connected, methodName, args);
				...
				if (isOverridesObject(lastItem)) {
					...
					finalCallArgs = [...argsWithoutOverrides, newOverrides] as ContractMethodArgs<TContract, TMethodName>;
				} else {
		...
		onError: async (error: TransactionError, variables) => {
			...
			console.error(`Error calling ${methodName as string} on: ${contract?.address} with arguments:`, variables, error);
			await onError?.(error);
		},
...
async function fetchGasEstimate<TContract extends Contract, TMethodName extends ContractMethodName<TContract>>(
	connected: TContract,
	methodName: TMethodName,
